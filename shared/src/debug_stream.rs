...
static DEBUG_STREAM: std::sync::OnceLock<Arc<DebugStream>> = std::sync::OnceLock::new();

/// Initialize the global debug stream
pub fn init_debug_stream(max_messages: usize) -> Arc<DebugStream> {
    DEBUG_STREAM
        .get_or_init(|| Arc::new(DebugStream::new(max_messages)))
        .clone()
}

/// Get the global debug stream instance
pub fn get_debug_stream() -> Option<Arc<DebugStream>> {
    DEBUG_STREAM.get().cloned()
}

/// Convenience functions for logging
pub async fn log_info(source: &str, message: &str) {
    if let Some(stream) = get_debug_stream() {
        stream.info(source, message).await;
    }
}

pub async fn log_warn(source: &str, message: &str) {
    if let Some(stream) = get_debug_stream() {
        stream.warn(source, message).await;
    }
}

pub async fn log_error(source: &str, message: &str) {
    if let Some(stream) = get_debug_stream() {
        stream.error(source, message).await;
    }
}

pub async fn log_debug(source: &str, message: &str) {
    if let Some(stream) = get_debug_stream() {
        stream.debug(source, message).await;
    }
}
